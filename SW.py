...
print("********************START OF EMD DECRYPTION********************")
SID = "1876007"
kWallet = SHA256(SID)
print("Current SID : " + SID)
print("Current kWallet: " + kWallet)
emd = encrypt(999, kWallet)
...
print("******************** START OF SMART WALLET UI********************")
...
while(choice != 0) :
	...
	if(choice == 3) : #Sync Wallets
		...
		senderID = input()
		cryptToken = generateToken(mySID[-3:], str(senderID), 0, 0)
		print("Sending token to..." + str(senderID) + "..." + cryptToken)
		m_count += 1
		synced = True;
		choice = 5
	...
	if(choice == 4): # Send Encrypted Token
		if(synced == True):
			...
			if(int(amount) > 999) :
				...
				print("Exception Thrown: You do not have enough money!")
